...
const sendSMS = async (data, callback) => {
    try {
        if (data.mobileNumber && data.mobileNumber.substring(0, 2) != '91') {
...
const sendMail = async (req, res) => {
    try {
        var transporter = nodemailer.createTransport({
            ...
            port: 465,
            auth: {
                user: 'actio.adiisha@gmail.com',
                pass: 'Act!o@2020'
            }
        });
        var mailOptions = {
            from: 'actio.adiisha@gmail.com',
            to: req.To,
            subject: req.Subject,
            text: req.Content
        };
...
module.exports = {
    teleSignSendSms, cleanNull, getAgeFromYYYYMMDD, iosFCM, generateRangeOfYears, convertFalsytoEmptyString,
    getdateTime, sendSMS, fcmAndroid, androidFCM, subscriberid, logError, sendMail, isValidDate, validateTime, uploadBase64, hasDuplicates, getAge, welcomeSMS
}