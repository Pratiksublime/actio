					...
					for (var j = 0; j < finaleventResult.length; j++) {
						...
						if(finalmatchLevelResult && typeof finalmatchLevelResult !== undefined && finalmatchLevelResult.length>0){
							for (var k = 0; k < finalmatchLevelResult.length; k++) {
								var statistic_str = "SELECT isd.* FROM individual_statistic as isd where isd.status = 1 AND isd.ml_id = "+finalmatchLevelResult[k].id+"";
...
module.exports = { performancelist_byid,individual_performance_master ,Awardlist,clublist,individualgallerylist,individualvideoslist}