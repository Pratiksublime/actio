...
const sendreminder = async (req, res) => {
    ...
    try {
        ...
        let notifyStr = "SELECT rp.id,rp.team_id,rp.player_name,rp.player_email,r.register_name,t.tournament_name,r.subscriber_id,r.event_id from registration_playre as rp left join registration as r on r.id = rp.team_id left join tournament as t on t.id = r.tournament_id  WHERE r.player_email = "+req.body.player_email+" and r.id= "+req.body.id+" ";
        const notify = await db.query(notifyStr);
        result.notify = notify.rows;
    } catch (err) {
        result.notify = [];
...
module.exports = {
    ...
    logEmail, playerEmailStatus, notify, notifySent,sendreminder 
}
