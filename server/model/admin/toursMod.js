...
console.log(req.body.sport_id);
console.log('req.body.featured_data.............1111111111111111111111111');
console.log(req.body.featured_data)
   ...
   //          }
			...
			if(req.body.gallery){
				...
				if(Array.isArray(gallerydata)) {
									console.log("uploaded121221212121211121212121212121212121212121212121212");
						...
						var gallerydata_str = null;
							...
							if(gallerydata[k].image) {
								...
								if(isImage) {
									...
									console.log("uploaded1212212121212111"); 
									console.log(isImage)
						    ...
						    let insertstr = "INSERT into tours_gallery(tour_id,media_type,title,sport_id,date,image,video_url ,created_at,status) values (" + tour_id + ",'" + gallerydata[k].media_type + "','"+gallerydata[k].title+"','"+gallerydata[k].sport_id+"','"+gallerydata[k].date+"','" + gallerydata_str + "','" + gallerydata[k].video_url + "',now(),1)";
						     console.log("insertstr///////////////////////////////////////////0000000") 
					...
					let insertstr = "INSERT into tours_gallery(tour_id,media_type,title,sport_id,date,image,video_url,created_at,status) values (" + tour_id + ",'" + gallerydata.media_type + "','"+gallerydata.title+"','"+gallerydata.sport_id+"','"+gallerydata.date+"','" + gallerydata_str + "','" + gallerydata.video_url + "',now(),1";
...
console.log('req.files.image..........22222777777777777777777');
     	...
     	//let img = req.files.image;
     	 //console.log(img);
     	// console.log('req.files.title..........111111');  
     	 //console.log(req.body.title)
     		
     		 
			 if ( req.files == null ||  req.files.length == 0) { 
			// console.log('fdfdfsd4444444444');
			// var element = req.files.image;


			// console.log('lkkkkkkkkkkkkkkkkkkkkkkk');  
			// console.log(element);
...
try{
	console.log("0000000000000000000000");
     ...
     let insertstrde = "delete from tours_downloads where tour_id = "+req.body.tour_id+" ";
     ...
     if(Array.isArray(req.body.title)){
     	...
     	for (var i = 0; i < downloadsStr.length; i++) {
			...
			if (req.files.image!== null || req.files.image!== undefined || req.files.image.length > 0 || req.files.image!== '')  {
        	...
        	console.log("insertstr788888888888888888888888"); 
         	console.log(insertstr);
		...
		// if(req.body.download_id && typeof req.body.download_id !== undefined && req.body.download_id !==""){

  //        	let insertstr = "UPDATE tours_downloads SET title = '" + req.body.title + "',image = '"+brochure_str+"',updated_at = now() WHERE id = "+req.body.download_id+" ";

		// 	console.log("insertstr2222222222222222222");
  //        	console.log(insertstr);
  ...
  //        }
     ...
     // }
        ...
        var brochure_strs = "";
		if(req.files.brochure && typeof req.files.brochure !== "undefined" && req.files.brochure.length !== "") {
        ...
        if(req.body.presentation_id && typeof req.body.presentation_id !== undefined && req.body.presentation_id !== ""){
            ...
            var insertstr = "UPDATE tours_Presentation SET brochure = '" + brochure_strs + "',updated_at = now() WHERE id = "+req.body.presentation_id+" ";
                }

        else{        

        var insertstr = "INSERT into tours_Presentation (tour_id,brochure,created_at,status) values (" + req.body.tour_id + ",'" + brochure_strs + "',now(),1)";
...
const list = async (req, res) => {
    ...
    try {
        ...
        let dataResult = await db.query(listStr);
		 ...
		 console.log(result)
		console.log('result11111111111111111111111111')
		// let infoStr = [];
...
const info = async (req,body) =>{
	...
	console.log(infodata); 
    var infodatastr = await db.query(infodata);
	 ...
	 if(infodatastr.rowCount ){
		...
		let info ="SELECT id,brochure as brochure_img,CASE WHEN brochure IS NULL THEN '' ELSE Concat('"+UploadFileLink+"images/tours/',brochure) END as brochure FROM tours_presentation where status = 1 and tour_id = "+req.body.id+"";
		...
		let infodowmload = [];
		let infodow = "SELECT id,title,CASE WHEN image IS NULL THEN '' ELSE Concat('"+UploadFileLink+"',image) END as image FROM tours_downloads where status = 1 and tour_id = "+req.body.id+"";
		...
		let infogall = [];
		let info_gal ="SELECT *,id,media_type,video_url,CASE WHEN image IS NULL THEN '' ELSE Concat('"+UploadFileLink+"',image) END as image FROM tours_gallery where status = 1 and tour_id = "+req.body.id+"";
 ...
 console.log(sportcont);
		...
		finalinfo['documents_data'] = (infodocument.rows); 
	     ...
	     var tour_inclusion = [];
		        ...
		        let tournamentImages = await db.query(`SELECT id,type,attachment FROM ${process.env.SCHEMA}.tour_attachment WHERE tour_id = ${req.body.id}`)
                if (tournamentImages.rowCount) {
                    ...
                    finalinfo['tour_logo'] = (logo.length) ? logo[0] : {};
		...
		if(req.body.gallery) {
           ...
           for (var i = 0; i < gallerydata.length; i++) {
			...
			if(gallerydata[i].id && gallerydata[i].id !== undefined && gallerydata[i].id !== ""){
				var gallerydata_str = '';
					if(gallerydata[i].image) {
						...
						//var element = gallerydata[i].image;
						//var image_name = moment().format('MMDDYYYYHHmmss') + element.name;

						//console.log('image_name=====================');
						//console.log(element);
						//element.mv('./images/tours/' + image_name);
						//gallerydata_str = image_name;
			...
			// } 
				...
				let insertstr = "INSERT into tours_gallery(tour_id,media_type,sport_id,date,title,image,video_url,created_at,status) values (" + tour_id + ",'" + gallerydata[i].media_type + "','"+ gallerydata[i].sport_id+"','"+ gallerydata[i].date+"','"+ gallerydata[i].title+"','" + gallerydata_str + "','" + gallerydata[i].video_url + "',now(),1)";
...
try{
	const result = {};
	const cityStr = "SELECT id,country_id,city_name AS city FROM city  where country_id = "+req.body.country_id+"";
...
const master = async (req,res) =>{
    ...
    try {
		...
		const sportStr = "SELECT id,sports_name AS sports FROM sports where status = 1 ORDER BY id";
