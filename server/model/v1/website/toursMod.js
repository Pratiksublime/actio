...
const list = async (req, res) => { 
    ...
    try {
        ...
        //let datestr =  `AND DATE(t.end_date) > '${date}'`;
        ...
        let listStr = "SELECT t.*, t.id,tt.tour_type as tour_type_name, t.sport_id,t.tour_name,t.no_of_day,t.country_id, c.name as country_name FROM tours as t left join country as c on c.id = t.country_id left join tours_type tt on t.tour_type = tt.id WHERE t.status = 1 "+sports_id+tours_type+location+tour_name+"ORDER BY end_date DESC"; 
...
//Concat('"+UploadFileLink+"', CASE WHEN (attachment  != '' or attachment ='null' or attachment = '') THEN '' else Concat(attachment) end) as attachment
                    ...
                    var tourquery = "SELECT type,CASE WHEN (attachment IS NULL or attachment = '' or attachment ='null') THEN '' ELSE Concat('"+UploadFileLink+"',attachment) END  as attachment FROM tour_attachment where status = 1 and tour_id = "+result[i].id+" ORDER by id DESC";
...
const ToursInfo = async (req,res) => {
    ...
    try{
        if(req.query.tour_id && typeof req.query.tour_id !== undefined && req.query.tour_id!==""){
            ...
            console.log(tourtypeadd);
            var data ={};
             data = await db.query(tourtypeadd);

            // for (var i = 0; i < data.rows.length; i++) {

        
            //     let tour_de = data.rows[i].tours_details
            //     console.log("tour_de........");
            //     console.log(tour_de);

            //     var decryptedData = decipher.update(tour_de,"base64","utf-8"); 
                
            //     console.log("decryptedData111111111....");
            //     console.log(decryptedData);

            //     decryptedData += decipher.final("utf8");
            //     console.log("decryptedData222222222222....");
            //     console.log(decryptedData);

            //     data.rows[i].tours_details=decryptedData;
            //     console.log('data.rows[i].tours_details......');
            //     console.log(data.rows[i].tours_details);
...
const ToursGalleryInfo = async (req,res) => {
    ...
    try{
        if(req.query.tour_id && typeof req.query.tour_id !== undefined && req.query.tour_id!==""){
            var tourtypeadd = "SELECT a.media_type,a.title,a.date, (CASE WHEN image IS NOT NULL THEN Concat('"+UploadFileLink+"',image) WHEN image = '' THEN '' ELSE '' END) as image FROM tours_gallery a WHERE a.media_type = 'image' and a.status = 1 and a.tour_id ="+req.query.tour_id; 
...
const ToursvideoInfo = async (req,res) => {
    ...
    try{
        if(req.query.tour_id && typeof req.query.tour_id !== undefined && req.query.tour_id!==""){
            var tourtypeadd = "SELECT a.media_type,a.video_url, (CASE WHEN cover_img IS NOT NULL THEN Concat('"+UploadFileLink+"',cover_img) WHEN cover_img = '' THEN '' ELSE '' END) as cover_img_new,a.title,a.date, b.sports_name FROM tours_gallery a left join sports b on a.sport_id:: int = b.id WHERE a.media_type = 'video' and a.status = 1 and a.tour_id ="+req.query.tour_id; 
            console.log(tourtypeadd);
            let data = await db.query(tourtypeadd);
            result = data.rows;    
        }
...
const ToursIternaryInfo = async (req,res) => {
    ...
    try{
        if(req.query.tour_id && typeof req.query.tour_id !== undefined && req.query.tour_id!==""){
            var tourtypeadd = "SELECT a.* FROM tours_iternary a WHERE a.status = 1 and a.tour_id ="+req.query.tour_id+" ORDER by day_number asc"; 
            console.log(tourtypeadd);
            let data = await db.query(tourtypeadd);
            result = data.rows;    
        }
