const db = require('../../../db');
	...
	const insertMet = async (req, res) => {
		try {
			...
			var inserting = '';
			let id = req.myID
			const {from_time,to_time,cal_date,activity,location} = req.body

	        inserting += `INSERT into meeting (from_time,to_time,cal_date,activity,location,subscriber_id,created_at, status) values ('${from_time}','${to_time}','${cal_date}','${activity}','${location}',${id},now(), 1) ;`;
	...
	const updateMet = async (req, res) => {
		try {
			console.log('Updating meeting...');
	
			let meetingId = req.body.id; // assuming the meeting ID is passed as a URL parameter
			const { from_time, to_time, cal_date, activity, location } = req.body;
	
			let updatingQuery = '';
			updatingQuery += `UPDATE meeting SET from_time = '${from_time}', to_time = '${to_time}', cal_date = '${cal_date}', activity = '${activity}', location = '${location}' WHERE id = ${meetingId};`;
			...
			if (updatingQuery) {
				...
				var updateResult = await db.query(updatingQuery); 
				await db.query(`COMMIT;`);
				return true;
			}
...
module.exports = { insert,list,update,info,Delete,insertMet,updateMet}