...
const UploadFileLink =process.env.HOST+process.env.PORT+'/';
    ...
    //                          }
                             ...
                             if (req.body.award_logo) {
								  	const isImage = /^data:image/.test(req.body.award_logo);
								  	if (isImage) {
								    const { award_logo } = req.body;
								    const profilePath = 'images/award/';
								    const uploaded = helper.uploadBase64(award_logo, profilePath);

								    sharp(uploaded.path)
								      .resize({ width: 800, withoutEnlargement: true })
								      .toBuffer()
								      .then(buffer => helper.uploadBuffer(buffer, profilePath))
								      .then(compressed => {
								        award_logo_new = compressed.path;

								        console.log(award_logo_new);
