        ...
        if (req.body.olympicDetail) {
           ...
           console.log(queryString);
            ...
            WHERE olympic_id = ${req.body.olympicDetail}`);
                ...
                let fee = await db.query(`SELECT * from olympic_fee WHERE olympic_id = ${req.body.olympicDetail}`)
                if (fee.rowCount) {
                    for (let item of fee.rows) {
                        if (item.logo) {
                            let logo = [];
                            logo.push(item.logo)
                            item.logo = logo;
                ...
                finalData['olympic_fee'] = fee.rows;
...
const olympic_details = async (req, res) => {
    const result = {};
    try {
    let listStr = "SELECT _t.id AS olympic_id,_t.olympic_name,_t.status,_t.olympic_description,to_char(_t.created_at,'DD-MM-YYYY') created,s.full_name,s.subscriber_id FROM  " + process.env.schema + ".olympic as _t INNER JOIN " + process.env.schema + ".subscriber AS s ON _t.created_by = s.id where  _t.status=1 and _t.id="+req.body.olympic_id;

        console.log('listStr000000000000000');
        console.log(listStr);
    ...
    //try {
        ...
        let olympicStr = `INSERT INTO ${process.env.SCHEMA}.olympic
            (olympic_sports,olympic_name,status,olympic_description,created_by,
             ...
             olympic_start_time,olympic_end_time,type,
             olympic_start_date,olympic_end_date     
             ${(olympicEarlyBirdEndDate) ? ',olympic_early_bird_end_date' : ''}
             )
             VALUES('${req.body.olympic_sports}','${req.body.olympic_name}',${req.body.status},'${olympic_description}',
                ${created_by},
                ${req.body.olympic_country},${req.body.olympic_state},${req.body.olympic_city},
                '${req.body.olympic_start_time}','${req.body.olympic_end_time}','${req.body.type}',
                '${olympicStartDate}','${olympicEndDate}'${olympicEarlyBirdEndDate}
                ) RETURNING id`;
        ...
        if (olympic.rowCount) {
            ...
            if (fee && !req.body.isolympicfree) {
                ...
                const insert = `INSERT INTO olympic_fee
                    (entry_fee,early_bird_entry_fee,currency,created_at,olympic_id,status) 
                    VALUES (${fee.entry_fees},${fee.early_bird_entry_fees},${fee.currency},now(),${olympic_id},1);`; 
