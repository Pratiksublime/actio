...
const individual_eventMast = async (req, res) => {
    try {
      ...
      if (req.body.event_logo) {
        var img = req.body.event_logo[0];
                               let isImage = /^data:image/.test(img);
                                  if (isImage) {
                                  let profilePath = 'images/performance/';
      ...
      console.log(insertteventmaster);
         ...
         var id = insertteventmasterData.rows[0].id


            if (req.body.event_images && typeof req.body.event_images !== "undefined" && req.body.event_images.length > 0) {
					  for (var i = 0; i < req.body.event_images.length; i++) {
					  	 let profilePath = 'images/performance/';
					    var uploadedImage = helper.uploadBase64(req.body.event_images[i], profilePath);
					    event_images = uploadedImage.path;
					    let imgquery = "INSERT INTO individual_event_images (individual_event_id, event_images) VALUES (" + id + ", '" + event_images + "')";
						console.log("imgquery......................");
					    console.log(imgquery)
					    await db.query(imgquery);  
					  }
					} else {
					  event_images = null;
					}
...
const individual_matchlevelMast = async (req, res) => {
    try {
		...
		var inserttmatchmaster = `INSERT INTO individual_match_level(master_id,event_id,match_no,match_level,participant_finel_result,participant_finel_score,win_participant,participant_one_logo,participant_one,participant_one_score,participant_one_game_one,participant_one_game_two,participant_one_game_three,participant_one_game_four,participant_one_game_five,participant_one_game_total,participant_one_sets_one,participant_one_sets_two,participant_one_sets_three,participant_one_sets_four,participant_one_sets_five,participant_one_sets_total,participant_one_description,participant_two_logo,participant_two,participant_two_score,participant_two_game_one,participant_two_game_two,participant_two_game_three,participant_two_game_four,participant_two_game_five,participant_two_game_total,participant_two_sets_one,participant_two_sets_two,participant_two_sets_three,participant_two_sets_four,participant_two_sets_five,participant_two_sets_total,participant_two_description,team_name_one,team_one_logo,team_one_score,team_one_description,team_one_game_one,team_one_game_two,team_one_game_three,team_one_game_four,team_one_game_five,team_one_game_total,team_one_sets_one,team_one_sets_two,team_one_sets_three,team_one_sets_four,team_one_sets_five,team_one_sets_total,team_two_name,team_two_logo,team_two_score,team_two_description,team_two_game_one,team_two_game_two,team_two_game_three,team_two_game_four,team_two_game_five,team_two_game_total,team_two_sets_one,team_two_sets_two,team_two_sets_three,team_two_sets_four,team_two_sets_total,team_finel_result,team_finel_score,win_team,level,level_timeing,level_discipline,level_parameter,level_value,level_position,level_point,created_at,status )
         ...
         let id = await db.query(inserttmatchmaster);
         ...
         id= id.rows[0].id;
				...
				var level_images = "";
					...
					if (req.body.match_images && typeof req.body.match_images !== "undefined" && req.body.match_images.length > 0) {
					  for (var i = 0; i < req.body.match_images.length; i++) {
					    var uploadedImage = helper.uploadBase64(req.body.match_images[i], bannerPath);
					    match_images = uploadedImage.path;
					    let imgquery = "INSERT INTO matchlevel_images (match_level_id, match_images) VALUES (" + id + ", '" + match_images + "')";
					...
					if (req.body.level_images && typeof req.body.level_images !== "undefined" && req.body.level_images.length > 0) {
					  for (var i = 0; i < req.body.level_images.length; i++) {
					    var uploadedImage = helper.uploadBase64(req.body.level_images[i], bannerPath);
					    level_images = uploadedImage.path;
					    let imgquery = "INSERT INTO matchlevel_images (match_level_id, level_images) VALUES (" + id + ", '" + level_images + "')";
					     console.log("imgquery imgquery imgquery...............");
					    console.log(imgquery)
					    await db.query(imgquery);
					  }
					} else {
...
const indimatchlevelMastlist = async (req, res) => {
    ...
    try {
		...
		if(masterid && typeof masterid !== undefined && masterid !== ''){
         ...
         where_Mast = "and iml.master_id ="+masterid+" ";
    	}
      ...
      if(req.body.event_id && typeof req.body.event_id !== undefined && req.body.event_id !==""){
        ...
        let listStr = "SELECT iml.*, l.level_name,lv.level_name as l_name, ev.event_name,e.event_other FROM individual_match_level as iml left join individual_event as e on e.id = iml.event_id left join level as l on l.id = iml.match_level::integer left join level as lv on lv.id = iml.level::integer  left join event as ev on ev.id = e.event_name:: int WHERE iml.status = 1 "+where_Mast+" ORDER by iml.match_no DESC" ; 

        console.log('listStr1111111111111111111111111111111111111');  
        console.log(listStr);
...
const indistacMastlist = async (req, res) => {
    ...
    try {
      ...
      var where_str = "";
      if(req.body.event_id && typeof req.body.event_id !== undefined && req.body.event_id !==""){
        where_str = " and ist.event_id = "+req.body.event_id+""
      }


        let listStr = "SELECT ist.*,ev.event_name,e.event_other,iml.match_level,l.level_name,iml.match_no FROM individual_statistic as ist left join individual_event as e on e.id = ist.event_id left join event as ev on ev.id = e.event_name:: int left join individual_match_level as iml on iml.id = ist.ml_id left join level as l on l.id = iml.match_level:: int  WHERE ist.status = 1 and ml_id ="+ml_id+"" ; 
    ...
    try {
        ...
        if (req.body.id && req.body.id !== "undefined" && req.body.id !== "") {
            ...
            if (prid && prid !== "undefined" && prid !== "") {
                ...
                var event = "SELECT ie.*,e.event_name,e.id as event_id from individual_event as ie left join event as e on e.id:: varchar = ie.event_name where ie.status = 1 and ie.master_id = " + prid + ""; 
            ...
            if (eventdata && eventdata !== 'undefined' && eventdata !== "") {
                ...
                //event_id = eventdata.rows[0].master_id;
                 

                if (event_id && event_id !== "undefined" && event_id !== "") {

                    var match_lavel = "SELECT iml.*,e.event_name,ie.event_other,l.level_name,lv.level_name as l_name from individual_match_level as iml left join individual_event as ie on ie.id = iml.event_id left join event as e on e.id = ie.event_name:: int  left join level as l on l.id = iml.match_level:: int left join level as lv on lv.id = iml.level:: int where iml.status = 1 and iml.master_id = " + event_id + " ORDER BY iml.match_no DESC"; 
            ...
            if (matchlaveldata && matchlaveldata !== " " && matchlaveldata.length > 0) {
                ...
                var statistic = "SELECT ims.*,e.event_name,ie.event_other,iml.match_no from individual_statistic as ims left join individual_event as ie on ie.id = ims.event_id left join event as e on e.id = ie.event_name:: int  left join individual_match_level as iml on iml.id = ims.ml_id where ims.status = 1  and ml_id in (" + ml_id_arry.toString() + ")";
...
const updatePerformance = async (req,res) => {
  ...
  try{
    ...
    if(typeof req.body.venue_id == "undefined"){ 
...
const listbysubscriberid = async (req, res) => {
    let result = {};
    try {
        let listStr = "SELECT ip.*,s.sports_name ,sb.sub_sports_name,v.venue_name,c.city_name,st.state_name,ct.name,t.tournament_name FROM individual_performance as ip LEFT join venue as v on v.id = ip.venue_id LEFT join sub_sports as sb on sb.sub_sports_id = ip.subsport_id LEFT join city as c on c.id = ip.city_id LEFT join state as st on st.id = ip.state_id LEFT join country as ct on ct.id = ip.country_id LEFT join sports as s on ip.sport_id = s.id left join tournament as t on t.id = ip.turnament_name:: int where ip.status = 1 and ip.subscriber_id = '"+req.body.subscriber_id+"' ORDER by ip.id DESC"; 
        let dataResult = await db.query(listStr); 
        result = dataResult.rows;
    } catch (err) {
        console.log("err: ");
