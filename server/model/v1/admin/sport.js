...
const notDelete = async(req,res)=>{
    ...
    try{
        ...
        if (id && id!=="" && id!== "undefined") {
            ...
            let myteamdata = "SELECT sport from subscriber_my_teams where status = 1 and sport  = "+id+"";
            let myteaminfo = await db.query(myteamdata);
            profile =  myteaminfo;

            let awardsdata = "SELECT sport_id from subscriber_play_awards where status = 1 and sport_id  = "+id+""; 
            let awardsinfo = await db.query(awardsdata);
            profile =  awardsinfo;

           let gallerydata = "SELECT sport_id from subscriber_highlighted_gallery where status = 1 and sport_id  = "+id+""; 
            let galleryinfo = await db.query(gallerydata);
            profile =  galleryinfo;

            let videosdata = "SELECT sport_id from subscriber_highlighted_videos where status = 1 and sport_id  = "+id+"";  
            let videosinfo = await db.query(videosdata);
            profile =  videosinfo;

        }

        if(req.body.olympic_sports_id){

            let olympicdata = "SELECT id from tournament where status = 1 and string_to_array(sports,',') && array['"+req.body.olympic_sports_id+"'] and olympics_sports ="+req.body.olympic_id+"";
            
            let olympicinfo = await db.query(olympicdata);
        ...
        if(req.body.champ_id){
            ...
            let champdata = "SELECT id from tournament where status = 1 and olympics_sports ="+req.body.champ_id+"";
        ...
        if(req.body.tournament_id){
            ...
            let tournamentdata = "SELECT id from event where status = 1 and tournament_id ="+req.body.tournament_id+"";
            
            let tournamentinfo = await db.query(tournamentdata);

          
            profile =  tournamentinfo;
        }
