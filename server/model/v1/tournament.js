...
const updateTournament = async (req, res) => {
    try {
        ...
        let no_team = "";
        if (req.body.no_team) {
            no_team = `no_team = ${req.body.no_team},`
            console.log('no_team');
            console.log(no_team);
        }
        let max_player = "";
        if (req.body.max_player) {
            max_player = `max_player = ${req.body.max_player},`
           
        }

        let min_player = "";
        if (req.body.min_player) {
            min_player = `min_player = ${req.body.min_player},`
        ...
        if (req.body.tournament_sponsers) {
            ...
            for (let i of sponsers) {
                if (!i.id) {
                    ...
                    var data = await db.query(`INSERT INTO ${process.env.SCHEMA}.tournament_sponsers(
                    name, logo, created_at, tournament_id)
                    VALUES ('${i.personName}', '${uploadPath}', now(), ${req.body.tournamentID})`)
                    console.log('data')
                    console.log(data)
