...
const eventdetailsList = async (req, res) => {
    try {
        const result = {};
        const tournamentWesbite = await websiteTournamentModel.eventdetailsList(req);
        result.status = process.env.STATUS_200;
        result.data = tournamentWesbite;
        res.status(result.status).send(result);
    } catch (err) {
...
module.exports = { insert, update, UpcomingList, pastList, withoutPastList,countList, detailsList, info ,Eventlist,championList,Tournamentlist_bychamp,sportList,locationList,UpcomingListNew,PastListNew,detailsListNew,eventdetailsList }