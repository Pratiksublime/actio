...
const allnotification = async (req, res) => {
    let result = await notificationMod.allnotification(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    result
...
const accesscodeList = async (req, res) => {
    try {
        const result = {};
        const Wesbite = await notificationMod.accesscodedata(req);
...
const seennotification = async (req, res) => {
    try {
        const result = {};
        const Wesbite = await notificationMod.seennotification(req);
...
module.exports = { requestnotification,list,allnotification,accesscodeList,seennotification }