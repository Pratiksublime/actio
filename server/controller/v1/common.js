...
const userMenu = async (req, res) => {
    try {
        ...
        const menu = await commonModel.userMenu(req.myID,req);
        return res.status(process.env.STATUS_200).send({ "menu": menu });
    } catch (err) {
        const error = {};
        error.status = process.env.STATUS_500;
        error.message = 'Something Went Wrong';
...
const suggestion = async (req, res) => {
    ...
    let result = await commonModel.suggestion(req.body);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}

const getRequestValidationToken = async (req, res) => {
    const token = jwt.sign({
        message: 'Validation Token valid only for 30 seconds only',
        createdAt: new Date()
    }, process.env.SECRET2, { expiresIn: 60 * 1 / 2 });
    await commonModel.getRequestValidationToken(req.ip);
    return res.status(process.env.STATUS_200).send({
        status: process.env.STATUS_200,
        msg: 'Token valid only for 30 seconds',
        token: token
    });
}
...
const sendClient = async (req, res) => {
    ...
    // try {
    //     jwt.verify(token,process.env.SECRET2);
    // }
    // catch(err) {
    //     return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_TEMP_422 , message : 'Sorry, the token passed is invalid or expired.Please request for new token !' });
    // }
    let result = await commonModel.sendClient(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}

const getRelations = async (req, res) => {
    let result = await commonModel.relation(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}

const getCalendarInformation = async (req, res) => {
    let result = await commonModel.getCalendarInformation(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}

const getSpecificCalendarInformation = async (req, res) => {
    let result = await commonModel.getSpecificCalendarInformation(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}


const bannerList = async (req, res) => {
    let result = await commonModel.bannerList(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}

const testimonialList = async (req, res) => {
    let result = await commonModel.testimonialList(req);
    return res.status(process.env.STATUS_200).send({ status: process.env.STATUS_200, result: result });
}
...
const academy_login = async (req, res) => {
    ...
    try {
         ...
         const user = await commonModel.academy_login(req);
        if (user.user) {
            const token = jwt.sign({
                username: req.body.username,
                id: user.user.id
...
module.exports = {
    getSpecificCalendarInformation, getCalendarInformation, getRelations, sendClient,academy_login,
    getRequestValidationToken, suggestion, login, forgotUsername, resendOTP, sendOTP, validateOTP, logout,
    userMenu, forgotPassword, validateForgotPassword, updatePassword, bannerList, testimonialList
}
