...
const subscriberModel = require('../../../model/v1/admin/subscriber');
...
// new api for web admin panel .............................

const EducationProfileInsert = async (req, res) => {
    let result = await subscriberModel.EducationProfileInsert(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({
...
const EducationProfileUpdate = async (req, res) => {
    let result = await subscriberModel.EducationProfileUpdate(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const addMySports = async (req, res) => {
    let result = await subscriberModel.addMySports(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const updateSport = async (req, res) => {
    let result = await subscriberModel.updateSport(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const addMyteam = async (req, res) => {
    let result = await subscriberModel.addMyteam(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const addMySponsor = async (req, res) => {
    let result = await subscriberModel.addMySponsor(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const updateMySponsorweb = async (req, res) => {
    let result = await subscriberModel.updateMySponsorweb(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const insertGallery = async (req, res) => {
    let result = await subscriberModel.insertGallery(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const updateGallery = async (req, res) => {
    let result = await subscriberModel.updateGallery(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const insertVideos = async (req, res) => {
    let result = await subscriberModel.insertVideos(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const updateVideos = async (req, res) => {
    let result = await subscriberModel.updateVideos(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
const deleteAll = async (req, res) => {
    let result = await subscriberModel.deleteAll(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({ 
...
module.exports = {
    ...
    getMyTeams, updateMyTeams, updateSportsProfile, push, bulk, updateProfile, getSportsProfile, updateProfessionProfile,addMySports,updateSport,addMySponsor,insertGallery,updateGallery,
    getProfessionProfile, updateProfileViewPublic,Mytournamentlist,Myeventlist_byid,deletesubscriberdata,EducationProfileInsert,EducationProfileUpdate,addMyteam,updateMySponsorweb,
    insertVideos,updateVideos,deleteAll
};