...
const careerModel = require('../../../model/v1/admin/mycareer');
...
const insertMycareer = async (req, res) => {
    let result = await careerModel.insertMycareer(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({
...
const updateCareer = async (req, res) => {
    let result = await careerModel.updateCareer(req);
    if (typeof result === 'object' && result.serverError) {
        return res.status(process.env.STATUS_200).send({ error_type: 'Server Error', error_message: result.error })
    }
    res.status(process.env.STATUS_200).send({
...
module.exports = { updateMycareer,getMycareer,insertMycareer,updateCareer } 