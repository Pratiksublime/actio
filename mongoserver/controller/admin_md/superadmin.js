...
module.exports = {
  ...
  list: async (req, res) => {
    ...
    try {
      ...
      let CollectionName = "Mainaccounts";
      let MainAccountsCollection = mongoose.model(CollectionName, Schema.Mainaccounts);
      let levelCollection = mongoose.model('leveldata', Schema.leveldata);

      console.log('req.myID: ', req.myID);

      if (req.myID && req.query.academyId && req.query.role_id && req.query.level_id) {



        result = await levelCollection.find({
          status: 1,
          subscriberID: req.myID,
          academy_id: req.query.academyId,
          id: req.query.level_id
        });



      } else if (req.myID && req.query.academyId) {
        result = await MainAccountsCollection.find({
          status: 1,
          subscriberID: req.myID,
          id: req.query.academyId
        });
      } else {
        result = await MainAccountsCollection.find({ status: 1 }).sort({ id: -1 });
      }
      ...
      console.log('sportMap..............................');
      let academys = await MainAccountsCollection.find({});

      const academyMap = {};
      academys.forEach(academy => {
        academyMap[academy.id] = academy.academy_name;
      });

      let pCollection = mongoose.model('staffdatas', Schema.staffdatas);
      let porsonin = await pCollection.find({});

      const porsoninMap = {};
      porsonin.forEach(pmy => {
        // Create an object to hold both staff_name and staff_last_name
        porsoninMap[pmy.id] = {
          staff_name: pmy.staff_name,
          staff_last_name: pmy.staff_last_name
        };
      });
      ...
      result = result.map((item) => {
        ...
        let sportNames = sportIds.map(id => sportMap[id] || 'Unknown');
        // let academy_name = academyMap[item.academy_id];
        let academyIdKey = item.academy_id ? 'academy_id' : 'id';


        let academy_name = academyMap[item[academyIdKey]];
        let person_inchargedata = porsoninMap[item.person_incharge];

        //let academy_name = academyMap[item.academy_name];
        console.log(academy_name);
        console.log('academy_nameacademy_nameacademy_name');

        return {
          ...item.toObject(),
          sportNames: sportNames,
          academy_name: academy_name,
          staff_name: person_inchargedata?.staff_name ?? null,
          staff_last_name: person_inchargedata?.staff_last_name ?? null,
        };
      });

      // Optimistic approach: Assuming positive outcome
      res.send({
        success: true,
      ...
      console.error(error);
      // If something goes wrong, then sending the actual error
      res.send({
        success: false,
        message: "something went wrong...",
        api_version: "v2",
  ...
  Edit: async (req, res) => {
    ...
    //     if (req.myID && req.body.academyId && req.body.role_id && req.body.level_id) {
    //         result = await levelCollection.find({
    //             status: 1,
    //             subscriberID: req.myID,
    ...
    //         });
    //     } else if (req.myID && req.body.academyId) {
    //         result = await dtmfCollection.find({
    //             status: 1,
    //             subscriberID: req.myID,
    //             id: req.body.academyId
    //         });
    //     } else {
